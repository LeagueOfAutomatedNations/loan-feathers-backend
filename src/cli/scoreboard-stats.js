...
async function run () {
  ...
  const { meta: { length: playerCount } } = await api.raw.scoreboard.list(0)
  const reqCnt = Math.ceil(playerCount / 20)
  const { time: gametime } = await api.raw.game.time('shardSeason')
  const { shards: [shardInfo] } = await api.raw.game.shards.info()
  const ps = []
  ...
  ps.splice(0, 1e5)
  const { seasons: [{ _id: season }] } = await api.raw.leaderboard.seasons()
  ...
  const { objects, users: oUsers } = await api.req('GET', 'https://screeps.com/season/api/game/room-objects?shard=shardSeason&room[$exists]=true')
